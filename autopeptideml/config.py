...
# class APML_Config:
#     """
#     Main configuration class for AutoPeptideML, combining different configurations.
#     """
#     def __init__(
#         self,
#         representation: dict,
...
#         if 'precalculated' in self.databases:
#             defaults = {"feat_fields": 'sequence', 'label_field': 'Y'}
#             for k, v in defaults:
#                 if k not in self.databases:
#                     print(f"Warning: {k} not specified, defaulting to {v}")
#                     self.databases[k] = v
#         elif 'dataset' not in self.databases:
#             raise ValueError("Please provide a training dataset.")
#         else:
#             defaults = {"feat_fields": 'sequence', 'label_field': 'Y',
#                         'verbose': False}
#             for k, v in defaults:
#                 if k not in self.databases['dataset']:
#                     print(f"Warning: Dataset {k} not specified, defaulting to {v}")
#                     self.databases['dataset'][k] = v

#         if 'neg_database' in self.databases:
#             defaults = {"feat_fields": 'sequence', 'label_field': 'Y',
#                         'verbose': False, 'seed': 1}
#             for k, v in defaults:
#                 if k not in self.databases['neg_database']:
#                     print(f"Warning: Neg database {k} not specified, defaulting to {v}")
#                     self.databases['neg_database'][k] = v

#     def _check_representation(self) -> None:
#         for idx, r in enumerate(self.representation['elements']):
#             name = list(r.keys())[0]
#             r_config = r[name]
#             if 'lm' in r_config['engine'].lower():
#                 defaults = {'average_pooling': True, 'batch_size': 12,
#                             'device': 'cpu'}
#                 if 'model' not in r_config:
#                     raise ValueError(f"Missing key `model` for representation config named: {name}")
#             elif 'fp' in r_config['engine'].lower():
#                 defaults = {'nbits': 2048, 'radius': 8}
#                 if 'fp' not in r_config:
#                     raise ValueError(f"Missing key `fp` for representation config named: {name}")

#             elif 'one_hot':
#                 if 'max_length' not in r_config:
#                     raise ValueError(f"Missing key `max_length` for representation config named: {name}")

#             for k, v in defaults.items():
#                 if k not in r_config:
#                     self.representation['elements'][idx][name][k] = v

#     def _check_pipeline(self) -> None:
#         if self.pipeline is None:
#             self.pipeline = {'name': 'Empty', 'elements': [],
#                              'aggregate': False}

#     def _check_train(self) -> None:
#         if 'optim_strategy' not in self.train:
#             raise ValueError("Please provide an optimization strategy. Key: `optim_strategy`.")
#         if 'hspace' not in self.train:
#             raise ValueError("Please provide a hyper-parameter optimisation space. Key: `hspace`.")

#         optim_strategy = self.train['optim_strategy']
#         if 'task' not in optim_strategy:
#             raise ValueError("Missing key: `task` in optim_strategy. Options: `class`, `reg`")
#         if optim_strategy['task'] == 'class':
#             defaults = {'direction': 'maximize', 'metric': 'mcc',
#                         'trainer': 'grid'}
#         elif optim_strategy['task'] == 'reg':
#             defaults = {'direction': 'maximize', 'metric': 'spcc',
#                         'trainer': 'grid'}
#         else:
#             raise NotImplementedError(f"Task {optim_strategy['task']} is not supported. Options: `class`, `reg`")

#         for k, v in defaults.items():
#             if k not in optim_strategy:
#                 self.train['optim_strategy'][k] = v

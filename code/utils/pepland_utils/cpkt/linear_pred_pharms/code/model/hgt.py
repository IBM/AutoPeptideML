...
class HGTLayer(nn.Module):
    def __init__(
        ...
        self.num_relations = len(edge_dict)-1
...
class HGT(nn.Module):
    def __init__(
        ...
        for k in node_dict.keys():
            if k == 'a':
                self.adapt_ws.append(nn.Linear(self.atom_dim,self.n_hid))
            if k == 'p':
                self.adapt_ws.append(nn.Linear(self.pharm_dim,self.n_hid))
    ...
    def forward(self, G):
        ...
        for ntype in G.ntypes:
            ...
            h[ntype] = F.gelu(self.adapt_ws[n_id](G.nodes[ntype].data["f"]))
        for i in range(self.n_layers):
            h = self.gcs[i](G, h)
        return self.out(h['a']),self.out(h['p'])

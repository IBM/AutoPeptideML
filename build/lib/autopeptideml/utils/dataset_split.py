...
def train_test(
    ...
    while not (test_size * 0.9 < division < test_size * 1.1):
        ...
        if not test_size * 0.9 < division < test_size * 1.1:
            print('Warning: Not enough independent data points for partitioning at current',
                  f' threshold/test_size values: {threshold:.2f}/{test_size:.2f}.',
                   f' Attempting partitioning at lower test_size: {test_size}')
            test_size -= 0.05

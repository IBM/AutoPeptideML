...
class AutoPeptideML:
    ...
    def compute_representations(self, datasets: dict, re: RepresentationEngine) -> dict:
        if self.verbose is True:
    ...
    def curate_dataset(self, dataset: str, outputdir: str=None) -> pd.DataFrame:       
        if self.verbose is True:
    ...
    def _make_figures(self, figures_path: str, truths, preds_proba):
        ...
        skplt.metrics.plot_lift_curve(truths, preds_proba, title='Lift Curve')
        plt.legend(loc='lower left')
        plt.savefig(os.path.join(figures_path, 'lift_curve.png'))
        skplt.metrics.plot_cumulative_gain(truths, preds_proba, title='Cumulative Gain Curve')
        plt.savefig(os.path.join(figures_path, 'cumulative_gain_curve.png'))
